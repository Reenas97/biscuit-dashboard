...
type Task = { id: string; title: string; date: string; endDate?: string; priority: TaskPriority; projectId?: string; completed: boolean; createdAt: string }
type TaskForm = { title: string; date: string; endDate: string; priority: TaskPriority; projectId: string }
...
const emptyTask: TaskForm = { title: '', date: '', endDate: '', priority: 'Média', projectId: '' }
...
function taskEndDate(task: Pick<Task, 'date' | 'endDate'>) { return task.endDate || task.date }
function formatTaskPeriod(task: Pick<Task, 'date' | 'endDate'>) {
  const start = parseDate(task.date).toLocaleDateString('pt-BR')
  const end = taskEndDate(task)
  return end !== task.date ? `${start} a ${parseDate(end).toLocaleDateString('pt-BR')}` : start
}
...
export function PlanningPage() {
  ...
  const monthTasks = useMemo(() => {
    ...
    return tasks
      .filter((task) => task.date <= monthEnd && taskEndDate(task) >= monthStart)
  ...
  const upcoming = useMemo(() => {
    const projectEvents = projects.filter((project) => project.deadline && project.status !== 'Stand by' && project.status !== 'Pronto' && project.status !== 'Entregue').map((project) => ({ id: `project-${project.id}`, title: project.title, date: project.deadline, kind: 'project' as const, completed: false }))
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const data = { title: form.title.trim(), date: form.date, endDate: form.endDate || form.date, priority: form.priority, projectId: form.projectId || undefined }
  ...
  function editTask(task: Task) {
    ...
    setForm({ title: task.title, date: task.date, endDate: taskEndDate(task), priority: task.priority, projectId: task.projectId ?? '' })
  }
  ...
  const isTaskLate = (task: Task) => !task.completed && taskEndDate(task) < today && !inactiveProjectIds.has(task.projectId ?? '')
  ...
  return <div className="planning-page">
    {selectedDate && selectedEvents && <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setSelectedDate(null) }}><section className="idea-modal day-details-modal" role="dialog" aria-modal="true" aria-labelledby="day-details-title"><div className="modal-heading"><div><span className="section-kicker"><FaCalendarDay /> AGENDA DO DIA</span><h2 id="day-details-title">{parseDate(selectedDate).toLocaleDateString('pt-BR', { weekday: 'long', day: '2-digit', month: 'long' })}</h2></div><button onClick={() => setSelectedDate(null)} type="button" aria-label="Fechar agenda do dia"><FaXmark /></button></div><div className="day-details-list">
      {selectedEvents.tasks.map((task) => { const linkedProject = projects.find((project) => project.id === task.projectId); return <article className={`day-detail-item task${task.completed ? ' completed' : isTaskLate(task) ? ' late' : ''}`} key={`task-${task.id}`}><div className="day-detail-icon"><FaClipboardCheck /></div><span><small>TAREFA · {task.priority}</small><strong>{task.title}</strong><p>{formatTaskPeriod(task)}{linkedProject ? ` · Projeto: ${linkedProject.title}` : ''}</p></span><button onClick={() => { setSelectedDate(null); editTask(task) }} type="button"><FaPen /> Editar</button></article> })}
      {selectedEvents.projects.map((project) => <article className={`day-detail-item project${project.status === 'Pronto' || project.status === 'Entregue' ? ' finished' : ''}`} key={`project-${project.id}`}><div className="day-detail-icon"><FaCalendarDay /></div><span><small>ENTREGA DE PROJETO</small><strong>{project.title}</strong><p>{project.client || 'Projeto pessoal'} · {project.status}</p></span><Link to="/projetos">Abrir <FaArrowRight /></Link></article>)}
    ...
    <div className="planning-layout mt-7">
      <section className="calendar-panel">
        <div className="calendar-heading"><div><span>CALENDÁRIO</span><h3>{month.toLocaleDateString('pt-BR', { month: 'long', year: 'numeric' })}</h3></div><div><button onClick={() => { setMonth(new Date(month.getFullYear(), month.getMonth() - 1, 1)); setTaskPage(1) }} aria-label="Mês anterior" type="button"><FaArrowLeft /></button><button onClick={() => { setMonth(new Date()); setTaskPage(1) }} type="button">Hoje</button><button onClick={() => { setMonth(new Date(month.getFullYear(), month.getMonth() + 1, 1)); setTaskPage(1) }} aria-label="Próximo mês" type="button"><FaArrowRight /></button></div></div>
        ...
        <div className="calendar-grid calendar-days">{days.map((day) => {
          ...
          return <div className={`${outside ? 'calendar-day outside' : 'calendar-day'}${key === today ? ' today' : ''}`} key={key} onClick={() => setSelectedDate(key)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') setSelectedDate(key) }} role="button" tabIndex={0} aria-label={`Ver compromissos de ${day.toLocaleDateString('pt-BR')}`}>
            ...
            <div className="calendar-events">
              {events.projects.slice(0, 2).map((project) => <Link className={`calendar-event project${project.status === 'Pronto' || project.status === 'Entregue' ? ' finished' : ''}`} to="/projetos" onClick={(event) => event.stopPropagation()} key={project.id} title={`${project.title} · ${project.status}`}>{project.title}</Link>)}
              {events.tasks.slice(0, 2).map((task) => <button className={`calendar-event task${task.completed ? ' completed' : isTaskLate(task) ? ' late' : ''}`} onClick={(event) => { event.stopPropagation(); saveTasks(tasks.map((item) => item.id === task.id ? { ...item, completed: !item.completed } : item)) }} type="button" key={task.id} title={`${isTaskLate(task) ? 'Tarefa atrasada: ' : ''}${task.title} · ${formatTaskPeriod(task)}`}>{task.title}</button>)}
      ...
      <aside className="planning-sidebar">
        ...
        <section className="tasks-panel"><div className="tasks-heading"><div><span className="section-kicker"><FaClipboardCheck /> TAREFAS DE {month.toLocaleDateString('pt-BR', { month: 'long' }).toLocaleUpperCase('pt-BR')}</span><h3>Lista do ateliê</h3></div><strong>{monthTasks.filter((task) => !task.completed).length}</strong></div>
          {monthTasks.length > 0 ? <><div className="task-list">{pageItems(monthTasks, taskPage, 10).map((task) => { const linkedProject = projects.find((project) => project.id === task.projectId); return <div className={task.completed ? 'task-row completed' : isTaskLate(task) ? 'task-row late' : 'task-row'} key={task.id}><button className="task-check" onClick={() => saveTasks(tasks.map((item) => item.id === task.id ? { ...item, completed: !item.completed } : item))} type="button" aria-label={`${task.completed ? 'Reabrir' : 'Concluir'} ${task.title}`}>{task.completed && <FaCheck />}</button><span><strong>{task.title}</strong><small>{formatTaskPeriod(task)} · <b className={`priority-text priority-${task.priority.toLocaleLowerCase('pt-BR')}`}>{task.priority}</b></small>{linkedProject && <Link className="task-project-link" to="/projetos">Projeto: {linkedProject.title}</Link>}</span><div className="task-row-actions"><button className="task-edit" onClick={() => editTask(task)} type="button" aria-label={`Editar ${task.title}`}><FaPen /></button><ConfirmButton className="task-delete" title="Excluir tarefa?" message={`A tarefa “${task.title}” será removida do planejamento.`} ariaLabel={`Excluir ${task.title}`} onConfirm={() => saveTasks(tasks.filter((item) => item.id !== task.id))}><FaTrash /></ConfirmButton></div></div> })}</div><Pagination page={taskPage} pageSize={10} totalItems={monthTasks.length} onPageChange={setTaskPage} /></> : <p>Nenhuma tarefa cadastrada neste mês.</p>}
        </section>
    ...
    {form && <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setForm(null) }}><section className="idea-modal" role="dialog" aria-modal="true" aria-labelledby="task-form-title"><div className="modal-heading"><div><span className="section-kicker"><FaClipboardCheck /> {editingTaskId ? 'EDITAR TAREFA' : 'NOVA TAREFA'}</span><h2 id="task-form-title">{editingTaskId ? 'Editar planejamento' : 'Adicionar ao planejamento'}</h2></div><button onClick={() => setForm(null)} type="button" aria-label="Fechar formulário"><FaXmark /></button></div><form onSubmit={handleSubmit}><label className="form-field form-field--full">Tarefa<input required value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} placeholder="Ex.: Terminar gatinhos" /></label><label className="form-field form-field--full">Projeto relacionado<select value={form.projectId} onChange={(event) => setForm({ ...form, projectId: event.target.value })}><option value="">Tarefa geral do ateliê</option>{projects.filter((project) => project.status !== 'Entregue').map((project) => <option key={project.id} value={project.id}>{project.title}</option>)}</select><small>Opcional: selecione o projeto ao qual esta tarefa pertence.</small></label><div className="form-grid"><label className="form-field">Início<input required type="date" value={form.date} onChange={(event) => setForm({ ...form, date: event.target.value, endDate: form.endDate && form.endDate >= event.target.value ? form.endDate : event.target.value })} /></label><label className="form-field">Término<input required min={form.date} type="date" value={form.endDate} onChange={(event) => setForm({ ...form, endDate: event.target.value })} /></label><label className="form-field form-field--full">Prioridade<select value={form.priority} onChange={(event) => setForm({ ...form, priority: event.target.value as TaskPriority })}><option>Baixa</option><option>Média</option><option>Alta</option></select></label></div><div className="modal-actions"><button className="secondary-button" onClick={() => setForm(null)} type="button">Cancelar</button><button className="primary-button" type="submit">{editingTaskId ? 'Salvar alterações' : 'Adicionar tarefa'}</button></div></form></section></div>}
