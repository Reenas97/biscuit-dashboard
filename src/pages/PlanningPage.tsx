...
type Task = { id: string; title: string; date: string; priority: TaskPriority; completed: boolean; createdAt: string }
type TaskForm = { title: string; date: string; priority: TaskPriority }
...
const emptyTask: TaskForm = { title: '', date: '', priority: 'Média' }
...
export function PlanningPage() {
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    saveTasks([{ id: crypto.randomUUID(), title: form.title.trim(), date: form.date, priority: form.priority, completed: false, createdAt: new Date().toISOString() }, ...tasks])
    setForm(null)
  }
  ...
  return <div className="planning-page">
    <div className="ideas-heading flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
      ...
      <div className="planning-actions"><button className="secondary-button" onClick={() => setUnavailableForm({ date: today, reason: '' })} type="button"><FaBan /> Dia indisponível</button><button className="primary-button shrink-0" onClick={() => setForm({ ...emptyTask, date: today })} type="button"><FaPlus /> Nova tarefa</button></div>
    </div>
    ...
    {form && <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setForm(null) }}><section className="idea-modal" role="dialog" aria-modal="true" aria-labelledby="task-form-title"><div className="modal-heading"><div><span className="section-kicker"><FaClipboardCheck /> NOVA TAREFA</span><h2 id="task-form-title">Adicionar ao planejamento</h2></div><button onClick={() => setForm(null)} type="button" aria-label="Fechar formulário"><FaXmark /></button></div><form onSubmit={handleSubmit}><label className="form-field form-field--full">Tarefa<input required value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} placeholder="Ex.: Comprar massa branca" /></label><div className="form-grid"><label className="form-field">Data<input required type="date" value={form.date} onChange={(event) => setForm({ ...form, date: event.target.value })} /></label><label className="form-field">Prioridade<select value={form.priority} onChange={(event) => setForm({ ...form, priority: event.target.value as TaskPriority })}><option>Baixa</option><option>Média</option><option>Alta</option></select></label></div><div className="modal-actions"><button className="secondary-button" onClick={() => setForm(null)} type="button">Cancelar</button><button className="primary-button" type="submit">Adicionar tarefa</button></div></form></section></div>}
